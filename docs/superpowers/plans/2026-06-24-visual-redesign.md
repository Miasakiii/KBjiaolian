...
- [x] **Step 1: 改 js（quickActions 换 svg 路径 + todaySummary 适配）**
...
- [x] **Step 3: 重写 wxss（只保留首页专用类，颜色全引用 token）**
...
- [x] 4 个自定义组件（kb-radar/kb-score/kb-quota/kb-empty）可用
- [x] app.wxss token 系统，页面颜色无硬编码草绿
- [x] 4 屏按 spec §7 重排
- [x] 26 个统一 svg 图标
- [x] 雷达图两变体正确
- [x] 无 emoji/字符占位
- [x] `--accent-warn` 仅用于问题点
