...
> 更新时间: 2026-06-27 | Sprint 1-4 全部完成 + 体验优化 + 代码审查修复（95 个问题）+ Flutter 移动端内测版完善（15 页）+ 微信支付对接 + 视觉设计系统跨端落地 + P0 安全加固 + P0/P1 全扫 + 小程序营养识别/进度趋势/渐进超负荷 + Flutter 数据导出 + CI/CD + Web 清理
...
**1. 验证码防爆破（`auth.js` + `database.js`）**
- DB 迁移：`verification_codes.attempts` 列
- `findActiveCode` SQL 不校验 code 值 → JS 层比对 + `incrementAttempts`
- 连续 5 次错误后 `markCodeUsed` 锁定，需重新获取

**2. authMiddleware 日志泄露修复（`auth.js`）**
- 删除 4 行 `console.log`（打印 headers/Authorization/JWT token）
- 保留不含 token 的 `console.error`（JWT 校验失败信息）

**3. Resend 邮件服务接入（新建 `email.js`）**
- `sendVerificationEmail(to, code, type)` — 开发模式 console.log fallback，生产模式 Resend API
- HTML 邮件模板：teal 品牌条 + 32px 验证码 + 5 分钟有效提示
- `auth.js` 中 `sendVerificationCode` / `forgotPassword` 两处替换为真实邮件发送
- `.env.example`：新增 `RESEND_API_KEY` / `MAIL_FROM`

**4. 数据库备份 + WAL checkpoint + 优雅关闭（`database.js` + 新建 `backup.js` + `index.js`）**
- 每 5 分钟 `wal_checkpoint(TRUNCATE)` 定时器（`unref()` 不阻止退出）
- `closeDatabase()` 函数：关闭前执行 checkpoint → `db.close()`（防双关）
- `backup.js`：启动即备份 + 每日 `copyFileSync` 到 `data/backups/` + 7 天自动清理
- `index.js`：`db.close()` → `closeDatabase()`，`import './backup.js'`

**验证：** 118 测试全部通过 + 启动确认备份生成

---

### ✅ 小程序功能扩展 + Flutter 导出 + CI/CD + Web 清理（2026-06-27）

本轮实施覆盖小程序三个新功能模块、Flutter 数据导出闭环、GitHub Actions CI 配置、Web 端遗留清理及文档修正：

**小程序 — 营养识别页面（`pages/nutrition/`）：**
- 餐次选择（早餐/午餐/晚餐/加餐）
- 拍照识别 + 结果展示（食物名称、营养成分估算）
- 今日营养汇总（卡路里/蛋白质/碳水/脂肪）
- 自动保存到后端

**小程序 — 进度趋势页面（`subpkg/history/progress/`）：**
- 统计卡片（分析次数/平均分/训练天数等）
- Canvas 折线图（分数趋势可视化）
- 历史记录列表

**小程序 — 渐进式超负荷集成（`pages/plan/`）：**
- 智能推荐按钮（基于训练历史）
- 建议面板（负荷调整建议展示）
- 渐进式方案生成 + 分享功能

**Flutter — 数据导出功能：**
- `ExportService` 统一导出服务（JSON 编码 + share_plus 系统分享）
- 首页导出按钮对接

**GitHub Actions CI 配置（`.github/workflows/ci.yml`）：**
- Backend job：安装依赖 + 运行测试
- Frontend job：构建检查
- Mobile job：Flutter analyze + test
- 三并行 job 架构

**Web 端遗留清理：**
- 删除孤立 PWA 文件（manifest.json / sw.js 等无后端配套的文件）
- 清理冗余依赖
- 修复下载链接

**文档修正：**
- `web/README.md` 重写（反映当前落地页状态）
- `backend/.env.example` 补全微信支付配置项
...
### 注册（邮箱验证码）
```
输入邮箱+密码 → POST /api/auth/send-code → 验证码通过 Resend 发送邮件
→ 输入验证码 → POST /api/auth/register → 创建用户+返回JWT
```
> 验证码 5 次错误后自动锁定，需重新获取。开发模式（无 RESEND_API_KEY）打印到控制台。
...
### 忘记密码
```
输入邮箱 → POST /api/auth/forgot-password → 验证码通过 Resend 发送邮件
...
- Web: tailwind config + 8 处 `emerald-500` 辅色待对齐 teal
...
| 端 | 完成度 | 说明 |
|----|--------|------|
| Backend | 92% | 核心功能完整，测试覆盖待提升 |
| Miniprogram | 92% | +7%：新增营养识别 + 进度趋势 + 渐进超负荷集成 |
| Mobile (Flutter) | 90% | +2%：数据导出闭环 |
| Web | 15% | 仅落地页，功能未开发 |
| 基础设施 | 92% | +7%：CI/CD + 文档修正 + Web 清理 |
| **项目整体** | **~89%** | 从 ~80% 提升 |
...
| 优先级 | 任务 | 说明 |
|--------|------|------|
| ✅ 已完成 | 小程序营养识别页面 | 餐次选择 + 拍照识别 + 结果展示 + 今日汇总 |
| ✅ 已完成 | Flutter 数据导出 | ExportService 统一导出 + 首页按钮对接 |
| ✅ 已完成 | 小程序进度趋势页 | 统计卡片 + Canvas 折线图 + 历史列表 |
| ✅ 已完成 | 文档修正与环境变量补全 | web/README 重写 + .env.example 补全微信支付配置 |
| ✅ 已完成 | 渐进式超负荷集成 | 小程序 plan 页智能推荐 + 建议面板 + 方案生成 + 分享 |
| ✅ 已完成 | CI/CD 配置 | GitHub Actions 三并行 job（Backend + Frontend + Mobile） |
| ✅ 已完成 | Web 端遗留清理 | 删除孤立 PWA 文件 + 清理冗余依赖 + 修复下载链接 |
...
| 🔴 P1 | 小程序测试基础设施 | 当前零覆盖，需引入测试框架 |
| 🟡 P1 | Backend 分支覆盖率提升 | 从 56.91% 提升至 70%+ |
| 🟢 P2 | 小程序恢复追踪/热力图页面 | 后端已有 API，小程序端待实现 |
| 🟢 P2 | 小程序首页 features 卡片跳转优化 | 登录后直接跳转对应功能页 |
