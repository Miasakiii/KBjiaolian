    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          disabled={isExporting}
          className="flex-1 py-3 px-4 bg-white hover:bg-gray-50 disabled:bg-gray-100 text-primary-700 font-medium rounded-xl border border-primary-200 transition-colors flex items-center justify-center gap-2"
        >
          {isExporting ? (
            <>
      ...
      <div className="fixed left-[-9999px] top-0">
        <div ref={reportRef} className="w-[800px] bg-white p-8">
          ...
          <div className="flex gap-8 mb-8">
            {imagePreview && (
              <div className="w-48 h-64 rounded-xl overflow-hidden border-2 border-gray-200">
                <img src={imagePreview} alt="分析照片" className="w-full h-full object-cover" />
              </div>
            )}
            <div className="flex-1">
              <div className={`${scoreBg} rounded-2xl p-6 mb-4`}>
                <div className="text-sm text-gray-600 mb-2">体态综合评分</div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { key: 'headForward', label: '头前伸', icon: '🦴' },
                { key: 'roundShoulder', label: '圆肩', icon: '💪' },
                { key: 'pelvicTilt', label: '骨盆前倾', icon: '🦵' },
                { key: 'kneeExtension', label: '膝超伸', icon: '🦿' },
              ].map((item) => {
                ...
                return (
                  <div key={item.key} className="p-4 bg-gray-50 rounded-xl">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="ml-auto text-sm font-bold text-gray-600">{value}%</span>
                    </div>
                ...
                <div key={i} className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-gray-800">{s.exercise}</span>
                    </div>
                  ...
                  {s.steps && s.steps.length > 0 && (
                    <div className="mb-2">
                      <div className="text-xs font-medium text-gray-500 mb-1">训练步骤：</div>
                  ...
                  {s.tips && s.tips.length > 0 && (
                    <div>
                      <div className="text-xs font-medium text-gray-500 mb-1">训练要点：</div>
          ...
          {/* 报告底部 */}
          <div className="pt-6 border-t-2 border-gray-200 text-center text-sm text-gray-500">
